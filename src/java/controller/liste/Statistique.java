...
public class Statistique extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Vente[] vente = new Vente().find("idmeuble="+idmeuble+" and idvolume="+idvolume, co.getConnectionPostgres());
            request.setAttribute("vente", vente);
            this.getServletContext().getRequestDispatcher("/Liste/statistique.jsp").forward(request, response);
        } catch (Exception ex) {
            ex.printStackTrace();
            out.print(ex);
